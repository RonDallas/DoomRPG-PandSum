    ...
    if (Player.Mission.Type == MT_ASSASSINATION)
    {
        ...
        for (int i = 1; i < MonsterID; i++)
        {
            ...
            if (StartsWith(Monsters[i].Actor, ActorToCheck, true))
                ...
                zsDynArrayUtilsSt("PotentialTargets", 1, i);
        ...
        if (zsDynArrayUtilsSt("PotentialTargets", 2, NULL)) // Proceed only if array is populated
        {
            // Randomly choose a valid monster to replace
            int Chosen = zsDynArrayUtilsSt("PotentialTargets", 3, NULL);
            ...
            // Cleanup
            zsDynArrayUtilsSt("PotentialTargets", 99, NULL);
        }
    }
}
